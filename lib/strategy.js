...
/**
 * @typedef {PlainObject} UserInfoStatus
 * @property {User} user
 ...
 * @param {Request} req Argument omitted if `passReqToCallback` is set
 * @param {string} username
 * @param {string} password
 * @param {VerifiedCallback} [verified]
 * @throws {Error} Synchronous errors will be passed to the `error` method of
 *   the `Strategy` object (as will any truthy first argument of `verified`)
 * @returns {void|Promise<UserInfoStatus>} If using the signature with
 *   `verified`, `void` may be returned and the `Promise` is expected
 *   otherwise.
 */
  ...
  */
  ...
  async authenticate (req, options) {
    ...
    */
     ...
     * @param {number} [status] A status code for failures (missing user)
     * @returns {void}
     */
    ...
    try {
      ...
      let error;
      try {
        result = await (this._passReqToCallback
